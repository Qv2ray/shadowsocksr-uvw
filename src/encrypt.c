...
static void dump(char *tag, char *text, int len)
{
    int i;
    printf("%s: ", tag);
    for (i = 0; i < len; i++) printf("0x%02x ", (uint8_t) text[i]);
    printf("\n");
}
...
static int crypto_stream_xor_ic(uint8_t *c, const uint8_t *m, uint64_t mlen, const uint8_t *n, uint64_t ic, const uint8_t *k, int method)
{
    switch (method)
    {
        case SALSA20: return crypto_stream_salsa20_xor_ic(c, m, mlen, n, ic, k);
...
int ss_encrypt(cipher_env_t *env, buffer_t *plain, enc_ctx_t *ctx, size_t capacity)
{
    if (ctx != NULL)
    {
        ...
        {
            err =
                cipher_context_update(&ctx->evp, (uint8_t *) (cipher->array + iv_len), &cipher->len, (const uint8_t *) plain->array, plain->len);
            if (!err)
            {
                return -1;
            }
...
void enc_table_init(cipher_env_t *env, int method, const char *pass)
{
    ...
    for (i = 0; i < 8; i++) key += OFFSET_ROL(digest, i);

    for (i = 0; i < 256; ++i) env->enc_table[i] = i;
    for (i = 1; i < 1024; ++i) merge_sort(env->enc_table, 256, i, key);
    for (i = 0; i < 256; ++i)
