...
// can be combined, en-mass, with the _NO_ forms later.
    ...
    #define CATCH_INTERNAL_SUPPRESS_PARENTHESES_WARNINGS _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Wparentheses\"")
...
#define INTERNAL_CATCH_TEST_CASE_METHOD2(TestName, ClassName, ...)                                                                              \
    ...
    {                                                                                                                                           \
        ...
        Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(Catch::makeTestInvoker(&TestName::test), CATCH_INTERNAL_LINEINFO, #ClassName,  \
...
namespace Catch
{
    ...
    struct StringMaker
    {
        ...
        static typename std::enable_if<::Catch::Detail::IsStreamInsertable<Fake>::value, std::string>::type convert(const Fake &t)
        {
        ...
        template<typename Fake = T>
        static typename std::enable_if<!::Catch::Detail::IsStreamInsertable<Fake>::value, std::string>::type convert(const Fake &)
        {
    ...
    namespace Detail
    {
        ...
        std::string rangeToString(InputIterator first, InputIterator last)
        {
            ...
            if (first != last)
            {
                ...
                for (++first; first != last; ++first) oss << ", " << ::Catch::Detail::stringify(*first);
            }
            oss << " }";
            return oss.str();
        }
...
namespace Catch
{
    ...
    struct StringMaker<std::pair<T1, T2>>
    {
        ...
        {
            std::ostringstream oss;
            oss << "{ " << ::Catch::Detail::stringify(pair.first) << ", " << ::Catch::Detail::stringify(pair.second) << " }";
            return oss.str();
        }
    };
} // namespace Catch
...
namespace Catch
{
    ...
    class BinaryExpr : public ITransientExpression
    {
        ...
        auto isBinaryExpression() const -> bool override
        {
            return true;
        }
      ...
      public:
        BinaryExpr(bool comparisonResult, LhsT lhs, StringRef op, RhsT rhs) : m_result(comparisonResult), m_lhs(lhs), m_op(op), m_rhs(rhs)
        {
        }
    };
...
// end catch_debugger.h
    ...
    #define INTERNAL_CATCH_TEST(macroName, resultDisposition, ...)                                                                              \
        ...
        } while (Catch::isTrue(false && static_cast<bool>(!!(__VA_ARGS__)))) // the expression here is never evaluated at runtime
          ...
          public:
            ...
            Approx &epsilon(T const &newEpsilon)
            {
                ...
                CATCH_ENFORCE(epsilonAsDouble >= 0 && epsilonAsDouble <= 1.0,
                              "Invalid Approx::epsilon: " << epsilonAsDouble << ", Approx::epsilon has to be between 0 and 1");
            ...
            Approx &margin(T const &newMargin)
            {
                ...
                CATCH_ENFORCE(marginAsDouble >= 0, "Invalid Approx::margin: " << marginAsDouble << ", Approx::Margin has to be non-negative.");
    ...
    class MatchExpr : public ITransientExpression
    {
      ...
      public:
        MatchExpr(ArgT const &arg, MatcherT const &matcher, StringRef matcherString)
            : m_arg(arg), m_matcher(matcher), m_matcherString(matcherString), m_result(matcher.match(arg))
        {
        }
    ...
    #define INTERNAL_CHECK_THAT(macroName, matcher, resultDisposition, arg)                                                                     \
        ...
        {                                                                                                                                       \
            Catch::AssertionHandler catchAssertionHandler(                                                                                      \
                macroName, CATCH_INTERNAL_LINEINFO, CATCH_INTERNAL_STRINGIFY(arg) ", " CATCH_INTERNAL_STRINGIFY(matcher), resultDisposition);   \
            INTERNAL_CATCH_TRY(catchAssertionHandler)                                                                                           \
            {                                                                                                                                   \
...
namespace Catch
{
    ...
    TestCase makeTestCase(ITestInvoker *testCase, std::string const &className, std::string const &name, std::string const &description,
...
namespace Catch
{
    ...
    void AssertionHandler::handle(AssertionResultData const &resultData, ITransientExpression const *expr)
    {
        ...
        if (!assertionResult.isOk())
        {
            ...
            m_shouldThrow = getCurrentContext().getRunner()->aborting() || (m_assertionInfo.resultDisposition & ResultDisposition::Normal);
        }
    }
            ...
            class BasicResult : public ResultValueBase<T>
            {
              ...
              protected:
                virtual void enforceOk() const
                {
                    ...
                    switch (m_type)
                    {
                        case ResultBase::LogicError: throw std::logic_error(m_errorMessage);
                        case ResultBase::RuntimeError: throw std::runtime_error(m_errorMessage);
                        case ResultBase::Ok: break;
                    }
                }
...
namespace Catch
{
    ...
    SourceLineInfo::SourceLineInfo(char const *_file, std::size_t _line) noexcept : file(_file), line(_line)
    {
    }
...
namespace Catch
{
    ...
    TestCaseStats::TestCaseStats(TestCaseInfo const &_testInfo, Totals const &_totals, std::string const &_stdOut, std::string const &_stdErr,
                                 bool _aborting)
...
namespace Catch
{
    ...
    StdErrRedirect::StdErrRedirect(std::string &targetString)
        : m_cerrBuf(cerr().rdbuf()), m_clogBuf(clog().rdbuf()), m_targetString(targetString)
    {
    ...
    RunContext::RunContext(IConfigPtr const &_config, IStreamingReporterPtr &&reporter)
        : m_runInfo(_config->name()), m_context(getCurrentMutableContext()), m_config(_config),
          m_reporter(std::move(reporter)), m_lastAssertionInfo{ "", SourceLineInfo("", 0), "", ResultDisposition::Normal }
    {
    ...
    void RunContext::assertionEnded(AssertionResult const &result)
    {
        ...
        // Reset working state
        m_lastAssertionInfo = { "", m_lastAssertionInfo.lineInfo, "{Unknown expression after the reported line}",
    ...
    void RunContext::runCurrentTest(std::string &redirectedCout, std::string &redirectedCerr)
    {
        ...
        catch (...)
        {
            ...
            {
                AssertionHandler(m_lastAssertionInfo.macroName, m_lastAssertionInfo.lineInfo, m_lastAssertionInfo.capturedExpression,
                                 m_lastAssertionInfo.resultDisposition)
                    .useActiveException();
            }
        }
...
namespace
{
    ...
    IStreamingReporterPtr makeReporter(std::shared_ptr<Config> const &config)
    {
        ...
        for (auto const &name : reporterNames) addReporter(reporter, createReporter(name, config));
        return reporter;
    }
    ...
    void addListeners(IStreamingReporterPtr &reporters, IConfigPtr const &config)
    {
        ...
        for (auto const &listener : listeners) addReporter(reporters, listener->create(Catch::ReporterConfig(config)));
    }
...
namespace Catch
{
    ...
    StringRef::StringRef(StringRef const &other) noexcept : m_start(other.m_start), m_size(other.m_size)
    {
    }

    StringRef::StringRef(StringRef &&other) noexcept : m_start(other.m_start), m_size(other.m_size), m_data(other.m_data)
    {
        other.m_data = nullptr;
    }

    StringRef::StringRef(char const *rawChars) noexcept : m_start(rawChars), m_size(static_cast<size_type>(std::strlen(rawChars)))
    {
        assert(rawChars != nullptr);
    }

    StringRef::StringRef(char const *rawChars, size_type size) noexcept : m_start(rawChars), m_size(size)
    {
    ...
    StringRef::StringRef(std::string const &stdString) noexcept : m_start(stdString.c_str()), m_size(stdString.size())
    {
    }
...
namespace Catch
{
    ...
    TestCase makeTestCase(ITestInvoker *_testCase, std::string const &_className, std::string const &_name, std::string const &_descOrTags,
...
namespace Catch
{
    ...
    AutoReg::AutoReg(ITestInvoker *invoker, SourceLineInfo const &lineInfo, StringRef classOrMethod, NameAndTags const &nameAndTags) noexcept
    {
        ...
        {
            getMutableRegistryHub().registerTest(
                makeTestCase(invoker, extractClassName(classOrMethod), nameAndTags.name, nameAndTags.tags, lineInfo));
        }
        catch (...)
        {
            // Do not throw when constructing global objects, instead register the
    ...
    struct CompactReporter : StreamingReporterBase<CompactReporter>
    {
      ...
      private:
        ...
        void printTotals(const Totals &totals) const
        {
            ...
            {
                stream << "Passed " << bothOrAll(totals.testCases.total()) << pluralise(totals.testCases.total(), "test case")
                       << " (no assertions).";
            }
            ...
            else
            {
                ...
                stream << "Passed " << bothOrAll(totals.testCases.passed) << pluralise(totals.testCases.passed, "test case") << " with "
...
namespace Catch
{
    ...
    namespace
    {
        ...
        class TablePrinter
        {
          ...
          public:
            TablePrinter(std::ostream &os, std::vector<ColumnInfo> const &columnInfos) : m_os(os), m_columnInfos(columnInfos)
            {
            }
            ...
            friend TablePrinter &operator<<(TablePrinter &tp, ColumnBreak)
            {
                ...
                auto padding =
                    (strSize + 2 < static_cast<std::size_t>(colInfo.width)) ? std::string(colInfo.width - (strSize + 2), ' ') : std::string();
        ...
        class Duration
        {
          ...
          public:
            Duration(uint64_t inNanoseconds, Unit units = Unit::Auto) : m_inNanoseconds(inNanoseconds), m_units(units)
            {
    ...
    struct ConsoleReporter : StreamingReporterBase<ConsoleReporter>
    {
        ...
        ConsoleReporter(ReporterConfig const &config)
            : StreamingReporterBase(config),
              m_tablePrinter(config.stream(), { { "benchmark name", CATCH_CONFIG_CONSOLE_WIDTH - 32, ColumnInfo::Left },
        ...
        void benchmarkEnded(BenchmarkStats const &stats) override
        {
            ...
            m_tablePrinter << stats.iterations << ColumnBreak() << stats.elapsedTimeInNanoseconds << ColumnBreak() << average << ColumnBreak();
        }
...
namespace Catch
{
    ...
    {
      public:
        JunitReporter(ReporterConfig const &_config) : CumulativeReporterBase(_config), xml(_config.stream())
        {
...
namespace Catch
{
    ...
    bool MultipleReporters::assertionEnded(AssertionStats const &assertionStats)
    {
        ...
        for (auto const &reporter : m_reporters) clearBuffer |= reporter->assertionEnded(assertionStats);
        return clearBuffer;
    }
...
namespace Catch
{
    ...
    {
      public:
        XmlReporter(ReporterConfig const &_config) : StreamingReporterBase(_config), m_xml(_config.stream())
        {
            m_reporterPrefs.shouldRedirectStdOut = true;
        }
...
// end catch_default_main.hpp
    ...
    // CATCH_
        ...
        #define CATCH_CHECK_NOFAIL(...)                                                                                                         \
            INTERNAL_CATCH_TEST("CATCH_CHECK_NOFAIL", Catch::ResultDisposition::ContinueOnFailure | Catch::ResultDisposition::SuppressFail,     \
                                __VA_ARGS__)
